...
/**
 * 功能描述 : freemarker自定义标签user
 * @author WeiQiangFan
 * @version [版本号,  2018/8/19]
 * @see [相关类/方法]
 * @since [产品/模板版本号]
 */
public class UserDirectiveModel extends AbstractElementDirectiveModel
{
    /**
     * Model Name
     */
    ...
    {
        if (null != this.getSubject() && null != this.getSubject().getPrincipal())
        {
            if (LOGGER.isDebugEnabled())
            {
                LOGGER.debug("The Subject has known identity (aka 'principal'). Tag body will be evaluated.");
            }
        ...
        else
        {
            if (LOGGER.isDebugEnabled())
            {
                LOGGER.debug("The Subject does not exist or have a known identity (aka 'principal'). Tag body will not be evaluated.");
            }
        }
    }
}