/**
 * 文件名: NotAuthenticatedDirectiveModel.java
...
/**
 * 功能描述 : freemarker自定义标签notAuthenticated
 * @author WeiQiangFan
 * @version [版本号,  2018/8/19]
 * @see [相关类/方法]
 * @since [产品/模板版本号]
 */
public class NotAuthenticatedDirectiveModel extends AbstractElementDirectiveModel
{
    /**
     * Model Name
     */
    public static final String MODEL_NAME = "notAuthenticated";
            ...
            throws TemplateException, IOException
    {
        if (!(null != getSubject() && getSubject().isAuthenticated()))
        {
            ...
            {
                LOGGER.debug("The subject is not authenticated. Tag body will be evaluated.");
            }
        ...
        else
        {
            if (LOGGER.isDebugEnabled())
            {
                LOGGER.debug("The subject is authenticated.  Tag body will not be evaluated.");
            }
        }
    }
}