/**
 * 文件名: HasAllPermissionsDirectiveModel.java
...
/**
 * 功能描述 : freemarker自定义标签hasAllPermissions
 * @author WeiQiangFan
 * @version [版本号,  2018/8/19]
 * @see [相关类/方法]
 * @since [产品/模板版本号]
 */
public class HasAllPermissionsDirectiveModel extends AbstractAttributeDirectiveModel
{
    /**
     * Model Name
     */
    public static final String MODEL_NAME = "hasAllPermissions";
            ...
            throws TemplateException, IOException
    {
        ...
        if (null != this.getSubject() && this.getSubject().isPermittedAll(permissions))
        {
            if (LOGGER.isDebugEnabled())
            {
                LOGGER.debug("The subject is all permitted. Tag body will be evaluated.");
            }
        ...
        else
        {
            if (LOGGER.isDebugEnabled())
            {
                LOGGER.debug("The subject is not all permitted. Tag body will not be evaluated.");
            }
        }
    }
}